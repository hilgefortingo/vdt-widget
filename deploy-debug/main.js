  ...
  class VDTDebugWidget extends HTMLElement {
    ...
    _processDataBinding() {
      ...
      if (db.data && Array.isArray(db.data)) {
        ...
        if (db.data.length > 0) {
          firstRowEl.textContent = JSON.stringify(db.data[0], null, 2);

          // Build data table for first 20 rows
          const keys = Object.keys(db.data[0]);
        ...
        const dimKeys = Object.keys(db.data[0]).filter(k => {
          const v = db.data[0][k];
        ...
        dimKeys.forEach(dimKey => {
          ...
          members.forEach(m => {
            membersHtml += m.id + ' — ' + (m.label || '(no label)');
