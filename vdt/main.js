...
(function () {
  ...
  class VDTEngine {
    ...
    // Return plain time member ID — the SAC script resolves it to the full
    // SAC format via getMembers() lookup (supports both leaf and parent levels)
  ...
  class ValueDriverTreeWidget extends HTMLElement {
    ...
    _publishChanges() {
      ...
      // Pass plain time IDs — the SAC script resolves them to full SAC member IDs
      // via ds.getMembers() lookup (handles both leaf and parent level formats)
      console.log("VDT: _publishChanges — " + entries.length + " entries, displayMode=" + (ctx.displayMode || "year"));
